...
public class Rwl_Employee implements Comparable<Rwl_Employee> {
	...
	public int compareTo(Rwl_Employee o) {
		if (this.name.length() > o.name.length()) {
			...
			if (this.equals(o)) {// duplicate remove sathi eqals method ahe
				return 0;
			} else {
				return this.name.compareTo(o.name); // ani length same asel tar asc order ne sort hoil
				// tya sathi he logic ahe
			}
		}
	}
}